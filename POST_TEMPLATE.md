...
Every `.md` or `.mdx` file in `content/md/` or `content/mdx/` must include YAML frontmatter at the top of the file.
...
### File placement
- Plain `.md` posts go in `content/md/` (synced from Obsidian). `.mdx` posts go in `content/mdx/`.
...
- Placing files outside `content/md/` or `content/mdx/` → not picked up by Contentlayer.
